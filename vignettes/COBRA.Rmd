...
output:
  rmarkdown::html_vignette:
    fig_caption: yes
vignette: >
  %\VignetteIndexEntry{COBRA User Guide}
...
COBRA is a package to calculate and visualize performance metrics for ranking and
...
the COBRAData object. Note that only the largest categories will be retained (the
...
is provided, COBRA will automatically add or remove colors.
...
COBRA contains an interactive shiny application, for interactive exploration of
...
Instructions tab for formatting instructions, or the COBRAData_to_text()
function).
